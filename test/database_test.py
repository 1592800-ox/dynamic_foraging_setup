...
reaction_time = np.arange(1, 5, len(trial_indices))
moving_speed = np.arange(500, 200, len(trial_indices))
...
upload_session(mouse_code=mouse_code, date=today, stage=stage, prob_set=-1, choices=choice_test, rewarded=rewards, trial_indices=trial_indices, left_P=leftP_test, right_P=rightP_test, reaction_time=reaction_time, moving_speed=moving_speed, cursor=cursor)